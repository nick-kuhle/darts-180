...
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| Git / delivery           | Private GitHub `main`, merged initial foundation [PR #1](https://github.com/nick-kuhle/darts-180/pull/1), CI workflow, lint/format/type/test/build commands, issue templates, contribution/security notices, Vercel configuration                                              | Open/review the prepared `camera-field-test` branch, verify its GitHub Actions result, and configure branch protections/required review policy. |
...
| Browser Camera Score     | HTTPS browser camera, manual four-double-bed calibration, transparent size/perspective/focus setup gate, volatile clear-board reference, temporal elongated-change/endpoint proposals, manual tip picker, deterministic score mapping, DartCard handoff, no-media debug export | Fixed-mount heuristic only; it is not a trained entry-point model, real-world accuracy evidence, or a silent auto-score path.                   |
...
1. [Initial foundation PR #1](https://github.com/nick-kuhle/darts-180/pull/1) is merged. Open and
   review the prepared `camera-field-test` branch, verify its GitHub Actions result, then enable
   branch protection and required-review policy. `CODEOWNERS` temporarily points at `@nick-kuhle`
   until an organization/team structure exists.
...
| 2026-09-07 | Created private-repository bootstrap `main` and merged [initial foundation PR #1](https://github.com/nick-kuhle/darts-180/pull/1). The browser field-test extension is prepared as a separate review branch.                                                                                                                  |
