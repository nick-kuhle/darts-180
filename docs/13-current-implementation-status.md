...
| Guided own-throw bootstrap | The visible app now separates **Live Scoring** from one guided **Data Lab**. Data Lab takes a board-only JPEG, records a pseudonymous setup/session ID and explicit self-capture-use statement, guides the five-point rim-junction/tip labels, and can download or explicitly save a reviewed trio through a guarded private Blob intake Function. `local_five_point_dataset.py` checks pairs/dimensions/profile/consent, accepts explicit blank-board anchor-only records, rejects duplicates, and creates session-disjoint numeric YOLO splits. | No real captures have been collected or independently reviewed yet. The private route is disabled until the existing Vercel project has a private Blob store and a server-only collection key. Data Lab manual steps are collection tooling only, never a normal Live Scoring setup. |
...
- **No public Vercel redeployment has been made.** The existing production URL stays unchanged and still
  correctly reports that a verified strict model package is unavailable. The new private Data Lab intake
  Function is local code only and fail-closed until the owner configures a private Blob store and
  server-only collection key in that same project.
...
# 48 web tests plus TypeScript workspace tests and the Vite production build
...
and the Data Lab private-intake fail-closed configuration, authorization, bounded body, private-write, and
sanitized-error boundaries. The added Python tests cover development manifest creation, audit gates, session
ID validation, and local JPEG/sidecar-to-YOLO compilation with duplicate/wrong-profile rejection, explicit
blank-board anchors, and the corpus-wide dart-label requirement—without requiring GPU libraries. Existing ML
lint/format findings were also resolved under the project `ruff>=0.9` quality extra. Rust/native code is
unchanged; Cargo is not available in this environment for an additional native check.
...
1. **Configure and test the private collection gate.** Merge/deploy through the existing Vercel project,
   create its private Blob store and server-only collection key, then make one safe blank-board and one
   dart-test Data Lab record. Follow [`20-private-capture-lab.md`](20-private-capture-lab.md); do not
   send credentials, token-bearing links, raw archives, or media in chat/Git.
