...
**Status:** foundation, playable scoring prototype, and local data-tooling slice complete; no
production auto-scoring claim.
...
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------- |
| Git / delivery           | Private GitHub `main` bootstrap, initial foundation PR [#1](https://github.com/nick-kuhle/darts-180/pull/1), CI workflow, lint/format/type/test/build commands, issue templates, contribution/security notices, Vercel configuration | Review/merge PR #1, verify its GitHub Actions result, and configure branch protections/required review policy.                         |
...
`npm run verify` checks formatting, documentation links, all workspace TypeScript typechecks, web/rules/session/API tests, and the production web build. The Python suite covers canonical geometry, manifest/sidecar validation, synthetic generation, pose-quality heuristics, and the temporal baseline. Rust parity remains configured for GitHub Actions; Cargo is not available in this local environment.
...
1. Review and merge [initial foundation PR #1](https://github.com/nick-kuhle/darts-180/pull/1);
   verify its GitHub Actions result; then enable branch protection and required-review policy.
...
2. Import the repository into Vercel and manually test the deployed site on actual iOS and Android
   HTTPS browsers: camera permission, still capture, privacy reset, JPEG/manifest pairing, Annotation
   Lab import/pairing, and sidecar download.
...
5. Connect ranked real candidates to DartCard confirmation/correction events and retain manual
   fallback whenever quality or uncertainty gates fail.
...
- Darts 180 does **not** currently auto-score a dart from a phone camera.
- Capture/Annotation Labs do **not** upload raw media or establish training-data approval.
...
| Date       | Change                                                                                                                                                                                                                                                                                                               |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-07 | Created private-repository bootstrap `main` and opened [initial foundation PR #1](https://github.com/nick-kuhle/darts-180/pull/1). The PR contains the complete validated implementation snapshot and is awaiting review/merge.                                                                                      |
