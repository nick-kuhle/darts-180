...
| Own-throw bootstrap      | Data Lab now assigns a pseudonymous setup/session ID and requires an explicit self-capture-use statement per export. Annotate has a separate five-point label profile whose four rim-junction anchors exactly match the development engine. `local_five_point_dataset.py` checks pairs/dimensions/profile/consent, rejects duplicates, and creates session-disjoint numeric YOLO splits. | No real captures have been collected or independently reviewed yet. Manual Data Lab/Annotate steps are collection tooling only, never a normal Camera Play setup.                                                                |
...
# web (34), rules (25), vision-session (6), and API (2) tests + Vite production build
# model verifier result: production sentinel valid; optional five-point package absent (informational)

cd ml && PYTHONPATH=src python3 -m unittest discover -s tests -v   # 25 tests
...
and the five-point annotation profile/engine agreement. The added Python tests cover development manifest
creation, audit gates, session ID validation, and local JPEG/sidecar-to-YOLO compilation with duplicate and
wrong-profile rejection—without requiring GPU libraries. Existing ML lint/format findings were also resolved
under the project `ruff>=0.9` quality extra. Rust/native code is unchanged; Cargo is not available in this
environment for an additional native check.
...
1. **Collect the first own-throw bootstrap batch.** Use Data Lab + the five-point Annotate profile for
   12–20 board-only stills across at least three pseudonymous setup sessions, then compile them locally.
   Do not send credentials, token-bearing links, raw archives, or media in chat/Git.
2. **Grow a balanced first training set.** Build roughly 200–300 clearly labeled darts across 10+ setup
...
3. **Train/export and inspect the development artifact.** Supply a locally licensed base checkpoint, run
...
4. **Install and test only the development package.** Copy the reviewed ONNX + schema-1 manifest to the
