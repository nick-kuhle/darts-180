...
| Browser Camera Play      | HTTPS browser camera and embedded/insecure/permission diagnostics; automatic conventional red/green board-color finding with two-fit stability, a visible `20`-up guide, internal outer-double mapping, one-action **Start Play** baseline capture, temporal dart-shape proposals, deterministic internal endpoint ranking, direct in-camera turn confirmation, and ordinary DartCard correction. Visual gestures, named anchors/manual analysis/tip recovery/no-media debug export are optional recovery/advanced controls. A mounted hidden frame canvas fixes the prior “complete frame” dead end. | Fixed-mount heuristic only; it is not a trained board/number-orientation or entry-point model, real-world accuracy evidence, or a silent production auto-score path. The same normal flow does not branch on steel/soft-tip, but that is not a physical accuracy claim. Color bands cannot uniquely resolve every board rotation, so automatic mode currently assumes a level 20-up board. Direct top-level HTTPS browser testing is required; Arena/in-app previews cannot request camera permission. |
...
1. [Initial foundation PR #1](https://github.com/nick-kuhle/darts-180/pull/1),
   ...
   [Vercel/mobile deployment PR #3](https://github.com/nick-kuhle/darts-180/pull/3) are merged.
   Review [no-calibration Camera Play PR #4](https://github.com/nick-kuhle/darts-180/pull/4), verify
   its GitHub Actions result, then enable branch protection and required-review policy. `CODEOWNERS`
   temporarily points at `@nick-kuhle` until an organization/team structure exists.
2. Import the repository into Vercel and manually test the deployed site on actual iOS and Android
   HTTPS browsers: camera permission, complete-frame capture, automatic red/green Board Found,
   visible `20` orientation, one-action empty-board Start Play, automatic one-dart proposal, direct
   turn confirmation, ordinary DartCard correction, steel/soft-tip interaction parity, optional
   recovery only when auto-find fails, still capture, privacy reset, JPEG/manifest pairing, Annotation
   Lab import/pairing, and sidecar download. Follow
