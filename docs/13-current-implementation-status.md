...
| Browser Camera Play      | HTTPS browser camera and embedded/insecure/permission diagnostics; touch-fitted outer-double guide with `20` orientation, drag/tap, pinch, twist, and independent edge handles; one-action in-memory baseline via **Calibrate & Play**; temporal dart-shape proposals; deterministic internal endpoint ranking; direct in-camera turn confirmation; and ordinary DartCard correction. Named anchors/manual analysis/tip recovery/no-media debug export remain behind an optional advanced control. | Fixed-mount heuristic only; it is not a trained entry-point model, real-world accuracy evidence, or a silent production auto-score path. The same normal flow does not branch on steel/soft-tip, but that is not a physical accuracy claim. Direct top-level HTTPS browser testing is required; Arena/in-app previews cannot request camera permission. |
...
1. [Initial foundation PR #1](https://github.com/nick-kuhle/darts-180/pull/1),
   ...
   Verify each GitHub Actions result, then enable branch protection and required-review policy.
   `CODEOWNERS` temporarily points at `@nick-kuhle` until an organization/team structure exists.
2. Import the repository into Vercel and manually test the deployed site on actual iOS and Android
   HTTPS browsers: camera permission, touch board fit (drag/tap, pinch, twist, all edge handles),
   `20` orientation, one-action clear-board calibration, automatic one-dart proposal, direct turn
   confirmation, ordinary DartCard correction, steel/soft-tip interaction parity, still capture,
   privacy reset, JPEG/manifest pairing, Annotation Lab import/pairing, and sidecar download. Follow
...
- Darts 180 does **not** currently have a trained or production auto-scoring model. The deployed
  browser scorer makes a fixed-camera frame-difference proposal with a deterministic internal
  endpoint ranking; it can be corrected before a visit is confirmed and has no real-world accuracy proof.
