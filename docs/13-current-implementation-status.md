...
| Data Lab auto capture       | While the camera runs, Data Lab loads only the optional same-origin, hash-verified five-point development manifest and runs one Worker-owned inference loop over the live preview (about one pass every 0.9s). Its adapter can save only declared class-1–4 anchors above the manifest floor and class-0 tips mapped through a complete learned pose. A record saves only when the four learned anchors form a safe pose and, for a dart record, at least one settled tip is detected; otherwise it is skipped, never invented. There is no marker-editing or per-still confirmation screen: settled-dart and clear-board records auto-save, and stale/cancelled loops dispose the Worker and cannot save a changed frame. The sidecar records final point sources plus `reviewMethod: learned-suggestion-auto-capture-v1` and a versioned model/SHA/data-kind/backend audit hint when a pass ran. | No package is currently installed, so the Lab leaves auto capture off rather than deriving anchors/tips from board geometry, image thresholds, or metadata. Browser-side source metadata is an audit clue only; restricted review remains the trust boundary before data admission.                                                           |
